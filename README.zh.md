...
# 直接启动模式（跳过项目选择和工具选择界面）
ai-cli -t kiro-cli                          # 在当前目录启动 kiro-cli
ai-cli -t claude -p "我的项目"               # 在配置的项目中启动 claude
ai-cli -t codewhale -d /path/to/project      # 在指定目录启动 codewhale
ai-cli -t kiro-cli --platform wsl           # 在 WSL 环境中启动
```
...
| 选项 | 简写 | 说明 |
|------|------|------|
| `--tool NAME` | `-t` | AI 工具名称（如 `kiro-cli`、`claude`）。传入即进入直启模式。 |
| `--project NAME` | `-p` | 配置中的项目名称（用于注入环境变量） |
| `--dir PATH` | `-d` | 工作目录路径（与 `-p` 互斥） |
| `--platform ENV` | — | 目标平台：`windows`/`wsl`/`linux`/`macos`（默认当前平台） |

当传入 `--tool` 时，AI-CLI 跳过交互界面，直接在当前终端运行工具，工具退出后 AI-CLI 也退出。如果匹配到项目配置，会自动注入项目环境变量。
...
传入 `--tool` 参数可跳过交互界面，直接在当前终端运行 AI 工具：

```bash
# 最简用法：在当前目录启动 kiro-cli
ai-cli -t kiro-cli

# 在配置中的项目里启动（自动注入项目环境变量）
...
- 如果当前目录**与已配置的某个项目匹配**，该项目会被自动选中。
- 如果当前目录**不是已配置的项目**，项目列表顶部会出现"当前目录"条目，无需事先配置即可启动工具。
